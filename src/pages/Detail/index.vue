    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImage="skuImageList" />
          <!-- 小图列表 -->
          <ImageList :skuImage="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
export default {
  ...
  methods: {
    changeActive(saleAttrValue, arr) {
      // 排他思想
      // console.log(arr);
      // 遍历数组将所有售卖属性的值isChecked设为0，没有高亮
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 而后将点击的那个属性设置为高亮
    ...
    normSku(e) {
      ...
      // 输入的值为字符串，* 1为强制类型转换
      let inpSku = e.target.value * 1;
      if (inpSku < 0 || isNaN(inpSku)) {
        this.skuNum = 1;
      } else {
        this.skuNum = parseInt(inpSku);
      }
    },
    // 加入购物车向服务器发送请求
    async updateShopCart() {
      ...
      try {
        ...
        // 01路由跳转
        // 02将产品信息传过去addcartsuccess路由组件
        //03如何传递？ 将商品信息存储在会话存储中，一般储存的是字符串
